...
open IN, "Lau_dsrA_protein.faa_blast_result_col2_unique_grep_nr_remove_SOH";
while(<IN>){
	chomp;
	my $line = $_;
...
my $gene = ""; 
open IN, "Lau_dsrA_protein.faa_blast_result.txt";
while (<IN>){
	chomp;
	my $line = $_;
	my @tmp = split (/\t/);
	if (!$gene){
		...
		open OUT, ">tmp/Lau_dsrA_protein.faa_blast_result_Subfile_$gene.txt";
...
open OUT, ">Lau_dsrA_protein.faa_blast_result_parsed.txt_blastp_new_nr_non-virus_tophit.txt";
