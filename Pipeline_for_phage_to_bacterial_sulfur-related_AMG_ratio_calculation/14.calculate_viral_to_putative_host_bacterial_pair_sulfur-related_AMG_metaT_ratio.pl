...
foreach my $metaG_id (sort keys %MetaG){
	if ($metaG_id eq "3300001683"){
		my %Depth = (); # gene => bam id => expression level
		my @Head = (); my @Bam = (); my %Gene_id = ();
		open IN, "/slowdata/data1/IMGVR/Transcriptomic_mapping/2nd_mapping/MetaT.RPKM.txt";
		while (<IN>){
			chomp;
			if (/^Head/){
				@Head = split (/\t/);
		...
		open OUT, ">$metaG_id.viral_to_bacterial.pair_info.dsrA_gene_metaT_ratio_result.txt";
		my $row=join("\t", @Bam);
		print OUT "Head\t$row\n";
